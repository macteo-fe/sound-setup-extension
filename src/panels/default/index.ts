...
function formatCheckResultsHtml(results: ConfigCheckResult[]): string {
    ...
    for (const { objectName, used, usedInCode, usedOnSceneOnly, unusedNotInScene } of results) {
        ...
        for (const key of used) {
            if (sceneOnly.has(key)) {
                ...
                const dynamicOnly = entry?.dynamicOnly;
    ...
    if (summary.total > 0) {
        ...
        const dynamicPart =
            summary.usedDynamicInCode > 0
                ? ` · ${summary.usedDynamicInCode} dynamic in code`
                : '';
        parts.push(
            `<div class="info">${summary.usedInCode} in code${dynamicPart} · ${summary.usedOnSceneOnly} on scene · ${summary.unused} unused</div>`,
        );
    }
    return parts.join('');
}
...
module.exports = Editor.Panel.define({
    ...
    ready() {
        ...
        if ($.results) {
            ...
            const allowTextSelection = (event: Event) => event.stopPropagation();
